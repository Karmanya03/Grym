...
pub const COMMON_PORTS: &[(u16, &str)] = &[
    ...
    (636, "LDAPS"), (993, "IMAPS"), (995, "POP3S"), (1025, "RPC"),
    (1080, "SOCKS"), (1433, "MSSQL"), (1521, "Oracle DB"),
    (1701, "L2TP"), (1723, "PPTP"), (2049, "NFS"),
    (2082, "cPanel"), (2083, "cPanel SSL"), (2181, "ZooKeeper"),
    (2375, "Docker API"), (2376, "Docker API SSL"),
    (27017, "MongoDB"), (3000, "Development"), (3128, "Squid Proxy"),
    (3306, "MySQL"), (3389, "RDP"), (3690, "SVN"),
    (4369, "Erlang Port Mapper"), (5432, "PostgreSQL"),
    (5555, "Android ADB"), (5601, "Kibana"), (5672, "RabbitMQ"),
    (5900, "VNC"), (5984, "CouchDB"), (6379, "Redis"),
    (6443, "Kubernetes API"), (7474, "Neo4j"),
    (8000, "HTTP Alt"), (8080, "HTTP Proxy"), (8443, "HTTPS Alt"),
    (8500, "Consul"), (9000, "HTTP Alt"), (9092, "Kafka"),
        ...
        handles.push(tokio::spawn(async move {
            ...
            let socket_addr = match addr.parse::<SocketAddr>() {
                ...
                Err(_) => return PortResult {
                    host: h, port: p, service: s, open: false,
                },
            };
            match timeout(dur, TcpStream::connect(socket_addr)).await {
                Ok(Ok(_)) => PortResult {
                    host: h, port: p, service: s, open: true,
                },
                _ => PortResult {
                    host: h, port: p, service: s, open: false,
                },
...
pub fn port_results_to_findings(results: &[PortResult]) -> Vec<Finding> {
    ...
    open_ports.iter().map(|port| {
        let mut finding = Finding::new(
            format!("Open port found: {}/{} ({})", port.port, port.service, port.host),
            AssetRef {
                identifier: format!("{}:{}", port.host, port.port),
                kind: "network".into(),
            },
            if port.port == 22 || port.port == 443 { Severity::Info } else { Severity::Low },
        ...
        finding.evidence.push(Evidence::redacted(
            "port-scan",
            format!("Port {} ({}) is OPEN on {}", port.port, port.service, port.host),
            format!("{}:{} - {}", port.host, port.port, port.service),
        ));
        finding.remediation = format!("Ensure {} service on port {} is properly secured and necessary.", port.service, port.port);
        finding
    }).collect()
}
