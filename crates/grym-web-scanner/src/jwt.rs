...
const COMMON_JWT_SECRETS: &[&str] = &[
    ...
    "secret123", "pass", "token", "jwt_secret", "my_secret",
    "key", "test", "test123", "super_secret", "changethis",
    "s3cr3t", "p@ssw0rd", "letmein", "abc123", "1234567890",
];
...
pub async fn check_jwt_config(
    ...
    let Ok(response) = client
        .get("grym-web-scanner", _url.clone(), TechniqueTier::StandardDetection)
        .await
    else {
        return Ok(findings);
    };
    ...
    if let Some(auth_value) = auth_header {
        ...
        if is_jwt(token)
            && let Some(jwt) = decode_jwt(token) {
                ...
                jwt_finding.categories.push("A07:2025-Authentication-Failures".into());
                ...
                if jwt.algorithm.to_uppercase() == "NONE" {
                    ...
                    alg_finding.categories.push("A07:2025-Authentication-Failures".into());
                    ...
                    alg_finding.remediation = "Disable the 'none' algorithm and always validate algorithm in server code.".into();
                    alg_finding.references.push("https://auth0.com/blog/critical-vulnerabilities-in-json-web-token-libraries/".into());
                ...
                if jwt.algorithm.to_uppercase().starts_with("HS") {
                    for secret in COMMON_JWT_SECRETS {
                        ...
                        if let Some(true) = try_hmac_verify(&msg, secret, &jwt._signature, &jwt.algorithm) {
                            ...
                            weak_finding.categories.push("A07:2025-Authentication-Failures".into());
    ...
    if let Ok(re) = Regex::new(r"eyJ[a-zA-Z0-9_-]+\.[a-zA-Z0-9_-]+\.[a-zA-Z0-9_-]+")
        && let Some(jwt_match) = re.find(&body_lower) {
            ...
            if let Some(jwt) = decode_jwt(token) {
                ...
                finding.categories.push("A02:2025-Security-Misconfiguration".into());
                finding.cwe_ids.push(200);
                finding.evidence.push(Evidence::redacted(
                    "jwt-exposure",
                    "JWT exposed in response body",
                    format!("Algorithm: {}, Claims: {}", jwt.algorithm, &jwt.payload.chars().take(100).collect::<String>()),
                ));
                finding.remediation = "Never expose JWT tokens in responses. Use them only in Authorization headers.".into();
