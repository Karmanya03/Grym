...
pub async fn check_host_header_injection(
            ...
            .await
        {
                ...
                || body_links_contain_host(&response.body, INJECTED_HOST)
            {
                let detail = if response
                    .headers
                    .get("location")
                    .map_or(false, |l| l.contains(INJECTED_HOST))
                {
                    "redirect-reflection"
                } else {
                    "body-reflection"
                ...
                finding.evidence.push(Evidence::redacted(
                    detail,
                    format!(
                        "Injected host '{}' via parameter '{}' reflected in response",
                        INJECTED_HOST, param
                    ),
                ...
                finding
                    .references
                    .push("https://owasp.org/www-community/attacks/Host_Header_Injection".into());
            ...
            .await
        {
                ...
                || body_links_contain_host(&response.body, INJECTED_HOST)
            {
                ...
                finding.evidence.push(Evidence::redacted(
                    "absolute-url-path",
                    format!(
                        "Absolute URL path '//{}/' reflected in response",
                        INJECTED_HOST
                    ),
    ...
    if findings.is_empty() && baseline.status >= 200 && baseline.status < 300 {
        ...
        ref_url
            .query_pairs_mut()
            .append_pair("referer", &format!("https://{}/", INJECTED_HOST));
        ref_url
            .query_pairs_mut()
            .append_pair("referrer", &format!("https://{}/", INJECTED_HOST));
            ...
            .await
        {
                ...
                || body_links_contain_host(&response.body, INJECTED_HOST)
            {
                ...
                finding.evidence.push(Evidence::redacted(
                    "referer-injection",
                    format!(
                        "Referer override with '{}' reflected in response",
                        INJECTED_HOST
                    ),
    ...
    if baseline.status >= 200 && baseline.status < 300 {
        ...
        if baseline.body.contains(hostname)
            || baseline
                .headers
                .get("location")
                .map_or(false, |l| l.contains(hostname))
        {
            let mut finding = Finding::new(
            ...
            finding
                .categories
                .push("A07:2025-Identification-and-Authentication-Failures".into());
            finding.cwe_ids.push(644);
            finding.evidence.push(Evidence::redacted(
                "host-reflection",
                format!(
                    "Hostname '{}' reflected in response without injection",
                    hostname
                ),
            ...
            finding
                .references
                .push("https://owasp.org/www-community/attacks/Cache_Poisoning".into());
            findings.push(finding);
        }
    }
                ...
                .await
            {
                if host_is_reflected(&response.body, &response.headers, INJECTED_HOST) {
                    ...
                    finding.evidence.push(Evidence::redacted(
                        ...
                        format!(
                            "Injected host '{}' reflected in password reset endpoint",
                            INJECTED_HOST
                        ),
                    ...
                    finding.references.push(
                        "https://owasp.org/www-community/attacks/Password_Reset_Poisoning".into(),
                    );
                    findings.push(finding);
                    break;
                }
