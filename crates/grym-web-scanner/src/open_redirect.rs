...
pub async fn check_open_redirect(
    ...
    let redirect_params = ["url", "redirect", "redirect_uri", "return", "return_to",
                            "return_url", "next", "next_url", "continue", "dest",
                            "destination", "goto", "target", "link", "href",
                            "out", "view", "page", "site", "path", "to"];

    let test_urls = ["https://evil.com", "http://evil.com", "//evil.com",
                      "https://attacker.net", "///evil.com", "https://evil.com/redirect"];
    ...
    for (param_name, _value) in &base_query {
        ...
        for target in &test_urls {
                ...
                .await
            {
                ...
                if is_redirect && (location.contains(target.trim_start_matches("//"))
                    || location.contains("evil.com")
                    || location.contains("attacker.net"))
                {
                    let mut finding = Finding::new(
                        format!("Open Redirect in parameter '{}' (redirects to {})", param_name, target),
                        AssetRef {
                    ...
                    finding.evidence.push(Evidence::redacted(
                        "open-redirect",
                        format!("Parameter: {}, Target: {}, Location: {}", param_name, target, location),
                    ...
                    finding.references.push("https://owasp.org/www-community/attacks/Open_redirect".into());
