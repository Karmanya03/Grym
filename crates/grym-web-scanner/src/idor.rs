        ...
        for test_id in &test_ids {
            let mut test_url = url.clone();
            {
                ...
                for (k, v) in &base_query {
                    let val = if k == param_name { test_id.to_string() } else { v.clone() };
                ...
                && response.status == 200 {
                    ...
                    if has_user_data {
                        let mut finding = Finding::new(
                            format!("Potential IDOR in parameter '{}' with value '{}'", param_name, test_id),
                            AssetRef {
                        ...
                        finding.evidence.push(Evidence::redacted(
                            "idor-probe",
                            format!("Parameter: {}, Value: {}, Status: {}", param_name, test_id, response.status),
                        ...
                        finding.references.push("https://owasp.org/www-community/attacks/Insecure_Direct_Object_Reference".into());
