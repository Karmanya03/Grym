...
const LB_SIGNATURES: &[(&str, &str)] = &[
    (r"X-Forwarded-For", "Load balancer detected - potential CL.TE vector"),
    (r"X-Forwarded-Host", "Load balancer/proxy routing detected"),
    (r"X-Forwarded-Proto", "Load balancer SSL termination detected"),
    ...
    (r"X-HTTP-Method-Override", "Method override header - potential smuggling vector"),
    (r"Access-Control-Request-Method", "CORS pre-flight - potential bypass vector"),
];
...
pub async fn check_http_smuggling(
                        ...
                        f.remediation =
                            "Configure the front-end and back-end to use a consistent parser for \
                             HTTP headers. Disable duplicate header forwarding."
                                .into();
                        f.references.push("https://portswigger.net/web-security/request-smuggling".into());
                        findings.push(f);
                    }
            }
        }
            ...
            if let Some(probed) = probed_url
                    ...
                    .await
                {
                    ...
                    if probe_response.status == 400 {
                        ...
                        let mut f = Finding::new(
                            format!("HTTP parsing edge case detected: {} (HTTP {})", description, status_code),
                            AssetRef {
                                identifier: url.to_string(),
                                kind: "web".into(),
                            },
    ...
    for vp in &verb_params {
        ...
        if let Ok(verb_resp) = client
            .get("grym-web-scanner", verb_url, TechniqueTier::StandardDetection)
            .await
            && verb_resp.status == 200 && response.status != 200 {
                let mut f = Finding::new(
                ...
                f.evidence.push(Evidence::redacted(
                    ...
                    format!("Parameter {} allowed method override (status: {})", vp, verb_resp.status),
                    format!("Method override: {} -> POST, status: {}", vp, verb_resp.status),
                ));
                f.remediation =
