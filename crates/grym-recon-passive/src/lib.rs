...
use grym_core::{Confidence, Finding, AssetRef, Severity, Evidence, HttpResponseSnapshot};
...
pub fn fingerprint_headers(response: &HttpResponseSnapshot) -> Vec<TechnologyFingerprint> {
    ...
    let header_checks: Vec<(&str, fn(&str) -> Option<(String, String, Option<String>, Option<String>)>)> = vec![
        ("server", |v| {
            let parts: Vec<&str> = v.split(['/', ' ']).collect();
            let name = parts.first().unwrap_or(&"unknown").to_string();
            let version = parts.get(1).map(|s| s.to_string());
            let cpe = version.as_ref().map(|ver| format!("cpe:2.3:a:{name}:{name}:{ver}:*:*:*:*:*:*:*"));
        ...
        ("x-aspnet-version", |v| Some(("ASP.NET".into(), v.to_string(), Some(v.to_string()), Some(format!("cpe:2.3:a:microsoft:asp.net:{}:*:*:*:*:*:*:*", v))))),
        ("x-generator", |v| {
            let name = v.split(['/', ' ']).next().unwrap_or("unknown").to_string();
            Some((name, v.to_string(), None, None))
        }),
        ("via", |v| Some(("Proxy/Via".into(), v.to_string(), None, None))),
        ("set-cookie", |v| {
            if v.to_lowercase().contains("asp.net") { Some(("ASP.NET".into(), v.to_string(), None, None)) }
            else if v.to_lowercase().contains("phpsessid") { Some(("PHP".into(), v.to_string(), None, None)) }
            else if v.to_lowercase().contains("jsessionid") { Some(("Java/J2EE".into(), v.to_string(), None, None)) }
            else if v.to_lowercase().contains("rails") { Some(("Ruby on Rails".into(), v.to_string(), None, None)) }
            else { None }
        }),
    ];
...
pub fn fingerprints_to_findings(
    ...
    fingerprints
        ...
        .map(|fp| {
            ...
            finding.remediation = format!("Ensure {} is up to date and properly configured.", fp.name);
